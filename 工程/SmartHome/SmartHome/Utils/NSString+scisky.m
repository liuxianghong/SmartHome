...
-(NSData *)hexToData{
    ...
    for (int i=0;i<[self length] / 2;i++)
    {
        ...
        plain[i] = strtoul([str UTF8String], 0, 16);
    }
    return [NSData dataWithBytesNoCopy:plain length:[self length] / 2 *sizeof(char)];
}

-(NSData *)hexToDataSwap{
    NSData *data = [self hexToData];
    long length = data.length;
    char *plain = malloc(length);
    for (int i =0; i<length; i++) {
        plain[i] = ((char *)[data bytes])[length - 1 - i];
    }
    NSData *newData = [NSData dataWithBytes:plain length:length];
    free(plain);
    return newData;
}
...
- (NSString *)MD5String{
    NSMutableString *str=[[self dataFromMD5].description mutableCopy];
    NSString *rangeStr1 = @"<";
    NSString *rangeStr2 = @">";
    NSString *rangeStr3 = @" ";
    NSRange range = [str rangeOfString:rangeStr1];
...
-(BOOL)isValidatePW
{
    ...
    NSString *PWCheck = @"(?!^\\d+$)(?!^[a-zA-Z]+$)([-`=\\\[\\];',./~!@#$%^&*()_+|{}:\"<>?]*).{6,16}";
    NSPredicate *PWTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",PWCheck];
